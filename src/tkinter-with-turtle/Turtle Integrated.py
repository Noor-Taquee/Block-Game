...
def traceFinger(event):
  ...
  deltaX = event.x - iX
  deltaY = event.y - iY
  if (abs(deltaY) > minSwipe or abs(deltaX) > minSwipe) and (
    time() - swipeStartTime
  ) <= swipeDuration:
...
def animate():
  ...
  if not pause and not gameOver and gameStarted:
    ...
    if y < bottomBorder or y > topBorder or x < leftBorder or x > rightBorder:
      ...
      if (
        abs(blockPos[0] - targetPos[0]) < blockSize * 5
        and abs(blockPos[1] - targetPos[1]) < blockSize * 5
      ):
        if dynamicSize:
...
def changeLayout():
  ...
  difficultyMsg = Label(
    infoPanel, text=difficulty, font=("times new roman", 10), fg="white", bg="black"
  )

  optionDifficultyMsg = Label(
    optionDifficultyPanel,
    text=difficulty,
  ...
  bn_medium = Button(
    difficultyPanel, text="medium", fg="white", bg="orange", command=f_medium
  )
  ...
  optionsBtn = Button(
    infoPanel, text="l l", font=("arial", 10), fg="white", bg="black", command=f_option
  )
